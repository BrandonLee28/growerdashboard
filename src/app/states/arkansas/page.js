...
const ArkansasChart = (props) => {
  const {
    stateName = "Arkansas",
    regionType = "county",
    regionIdProperty = "NAME",
    // These initial view coordinates roughly center on Arkansas.
    initialViewCoordinates = {
      longitude: -92.373123,
      latitude: 34.969704,
      zoom: 7,
    },
    // The stateBounds are estimated for Arkansas.
    stateBounds = [
      [-94.61, 33.01],
      [-89.64, 36.50],
    ],
    dataTypes = [
      {
        key: `../arkansas_data/arkansas_ESTAB.csv`,
        label: "ESTAB",
        valueColumn: "Total",
        description: "Total number of establishments by county in Arkansas",
        minValue: 0,
        maxValue: 300000, // adjust as needed
        formatValue: (value) => value.toLocaleString(),
        unit: "buildings",
      },
      {
        key: `../arkansas_data/arkansas_saidi_by_county_rounded.csv`,
        label: "SAIDI",
        valueColumn: "SAIDI",
        description: "System Average Interruption Duration Index (minutes) in Arkansas",
        minValue: 100, // adjust as needed
        maxValue: 400, // adjust as needed
        formatValue: (value) => value.toFixed(2),
        unit: "mins",
      },
      {
        key: `../arkansas_data/arkansas_saifi_by_county_rounded.csv`,
        label: "SAIFI",
        valueColumn: "SAIFI",
        description: "System Average Interruption Frequency Index (interruptions/year) in Arkansas",
        minValue: 1.0, // adjust as needed
        maxValue: 3.0, // adjust as needed
        formatValue: (value) => value.toFixed(2),
        unit: "freq",
      },
    ],
    geojsonPath = `../arkansas_data/ar_counties.geojson`,
  } = props;
        ...
        Papa.parse(csvText, {
          ...
          complete: (results) => {
            ...
            const dataMap = results.data.reduce((acc, row) => {
              // For counties, assume the region name is stored in the "County" field.
              const regionName =
